        ...
        void serialPortDataCallBack(object paramsSet, object data)
        {
            ...
            switch (a)
            {
                ...
                case PLDParams.PLDParamsFromGet.Upgrade:
                    switch (p1.MeasureParams.UpgradeStatus)
                    {
                        ...
                        case PLDParams.UpgradeStatus.LAST_DONE:
                            if (!Update.IsUpdate)
                            {
                                Application.Current.Dispatcher.Invoke(() => aggregator.SendMessage("程序升级成功!", "Main"));
                            }
                            Update.IsAccess = true;
                            break;
                ...
                case PLDParams.PLDParamsFromGet.SaveParam:
                    if (p1.Ack == 1)
                    {
                        Application.Current.Dispatcher.Invoke(() => aggregator.SendMessage("参数保存成功!", "Main"));
                       
                    }
                    else
                    {
                        Application.Current.Dispatcher.Invoke(() => aggregator.SendMessage("参数保存失败!", "Main"));
