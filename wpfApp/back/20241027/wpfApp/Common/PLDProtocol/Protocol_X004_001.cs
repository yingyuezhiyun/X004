...
namespace wpfApp.Common.PLDProtocol
{
    ...
    public class PLDParams
    {
        ...
        public AllMeasureParam AllMeasureParams = new AllMeasureParam();
        public class AllMeasureParam
        {
            public float[] LDCurr = new float[2];
            public float[] LDVol = new float[2];
            public float PwrTemp;
            public float PD;
            public float PDTemp;
            public float[] TECTemp = new float[4];
            public float[] TECPower = new float[4];
            public float[] TECCur = new float[4];
        ...
        public enum StatusFlags : UInt32
        {
            ...
            TRQ_Type = 1 << 18,    // 触发状态 1：外触发; 0：内触发
            Pulse_Type = 1 << 19,  // 脉冲类型 1：定频 0：变频
        }
        ...
        private (object ParamsGet, object result) ParseFrame(PLDRev data)
        {
            ...
            switch ((DEV_GET_CMD_TYPE)data.cmd)
            {
                ...
                case DEV_GET_CMD_TYPE.TEC4_SW:
                    paramsGet = PLDParams.PLDParamsFromGet.TEC4_SW; goto SWParam;
                SWParam:
                    result = data.data[0] == (byte)PLDParams.WorkType.ON ? (byte)1 : (byte)0;
                    break;
                ...
                case DEV_GET_CMD_TYPE.ALL_M:
                    {
                        ...
                        PLDParams pLDParams = new PLDParams();
                        pLDParams.AllMeasureParams.LDCurr[0]= BitConverter.ToInt16(data.data, 0) / 10.0f;
                        pLDParams.AllMeasureParams.LDCurr[1] = BitConverter.ToInt16(data.data, 2) / 10.0f;
                        pLDParams.AllMeasureParams.LDVol[0] = BitConverter.ToInt16(data.data, 4) / 10.0f;
                        pLDParams.AllMeasureParams.LDVol[1] = BitConverter.ToInt16(data.data, 6) / 10.0f;
                        pLDParams.AllMeasureParams.PwrTemp = BitConverter.ToInt16(data.data, 8) / 10.0f;
                        pLDParams.AllMeasureParams.PD= BitConverter.ToInt16(data.data, 10) / 10.0f;
                        pLDParams.AllMeasureParams.PDTemp = BitConverter.ToInt16(data.data, 12) / 10.0f;
                        for (int i = 0; i < 4; i++)
                        {
                            pLDParams.AllMeasureParams.TECTemp[i] = BitConverter.ToInt16(data.data, 14 + 6 * i) / 10.0f;
                            pLDParams.AllMeasureParams.TECPower[i] = BitConverter.ToInt16(data.data, 16 + 6 * i) / 10.0f;
                            pLDParams.AllMeasureParams.TECCur[i] = BitConverter.ToInt16(data.data, 18 + 6 * i) / 10.0f;
                        }
                        pLDParams.AllMeasureParams.Status = data.data[38]|((UInt32)data.data[39]<<8)| ((UInt32)data.data[40] << 16);
                        pLDParams.AllMeasureParams.Version = "V"+ data.data[41]+"."+ data.data[42] + "."+ data.data[43] + "."+ data.data[44];

                        pLDParams.time = data.time;
                        result = pLDParams;
                    }
                    break;
                case DEV_GET_CMD_TYPE.UPGRADE:
                    break;
                case DEV_GET_CMD_TYPE.FreqType:
                    paramsGet = PLDParams.PLDParamsFromGet.PulseType;
                    result = data.data[0] == (byte)PLDParams.PulseType.SPWM ? PLDParams.PulseType.SPWM : PLDParams.PulseType.NOR;
                    break;
                ...
                case DEV_GET_CMD_TYPE.S_LCM:
                    {
                        ...
                        pLDParams.LCMSet.PwrEn = (byte)(data.data[0] == (byte)PLDParams.WorkType.ON ? 1 : 0);
                        pLDParams.LCMSet.MotorEn = (byte)(data.data[1] == (byte)PLDParams.WorkType.ON ? 1 : 0);
                        pLDParams.LCMSet.MotorSpeed = BitConverter.ToUInt16(data.data, 2);
                        pLDParams.LCMSet.Fan = data.data[4];
                        pLDParams.LCMSet.PwrLimit = data.data[5];
                        result = pLDParams;
                    }
                    break;
                case DEV_GET_CMD_TYPE.M_LCM:
                    {
                        ...
                        pLDParams.LCMMeasure.MotorEn = (byte)(data.data[1] == (byte)PLDParams.WorkType.ON ? 1 : 0);
        ...
        public List<byte> CreateFrame(object paramsSet, params byte[] data)
        {
            ...
            switch (a)
            {
                ...
                case PLDParams.PLDParamsToSet.TRG_TYPE:
                    ...
                    break;
                case PLDParams.PLDParamsToSet.FreqType:
                    cmd = (byte)DEV_SET_CMD_TYPE.FreqType;
                    break;
        ...
        public List<byte> CreateQueryFrame(object paramsQuery)
        {
            ...
            switch (a)
            {
                ...
                case PLDParams.PLDParamsToQuery.TRG_TYPE:
                    ...
                    break;
                case PLDParams.PLDParamsToQuery.FreqType:
                    cmd = (byte)DEV_QUERY_CMD_TYPE.FreqType;
                    break;
