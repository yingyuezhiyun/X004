...
{
    public class Protocol_X002_002 : ICommunicationProtocol
    {
        enum DEV_CMD_TYPE
        {
            SET_CMD = 3,//设置参数
        ...
        enum DEV_SET_CMD_TYPE
        {
            ...
            LD_TYPE = 0x11, // LD类型 弃用
            OC,             // 软件过流 弃用
            HOC,            // 硬件过流
            M_R,            // 测量电阻
            MAX_CUR,        // 最大电流

            TEC_MAX_V = 0x21, // TEC最大电压
            TEC_MAX_C,        // TEC最大电流
            MIN_TEMP,         // 最小温度
            MAX_TMEP,         // 最大温度

            CUR_S_k = 0x31, // LD设定电流 标定系数 k
            CUR_S_b,        // LD设定电流 标定系数 b
            CUR_M_k,        // LD测量电流 标定系数 k
            CUR_M_b,        // LD测量电流 标定系数 b
            PW_CUR_k,       // LD电流-LD光功率 标定系数 k
            PW_CUR_b,       // LD电流-LD光功率 标定系数 b
            PIN_PW_k,       // LD光功率-PIN电流 标定系数 k
            PIN_PW_b,       // LD光功率-PIN电流 标定系数 b
            LASER_p,        // 横流模式 LD光功率 PID控制 系数p
            LASER_i,        // 横流模式 LD光功率 PID控制 系数i
            LASER_d,        // 横流模式 LD光功率 PID控制 系数d

            TEC_k = 0x41, // TEC 标定系数 k 弃用
            TEC_b,        // TEC 标定系数 b 弃用
            TEMP_OFFSET,  // TEC温度偏置

            AUTO_RUN = 0xC1, // 上电自动启动
            Save_Para = 0xD1, // 参数固化
            REST,             // 恢复出厂设置 弃用
        ...
        public List<byte> CreateQueryFrame(DevParamsFromGet paramsSet, params byte[] data)
        {
            ...
            switch (paramsSet)
            {
                ...
                case DevParamsFromGet.LDMeasureR: frame.Add((byte)DEV_GET_CMD_TYPE.LD_M_R); break;
                ...
                case DevParamsFromGet.VERSION: frame.Add((byte)DEV_GET_CMD_TYPE.VERSION); break;
                case DevParamsFromGet.BUILDTIME: frame.Add((byte)DEV_GET_CMD_TYPE.BUILDTIME); break;
                default: return new List<byte>();
            }
        ...
        DevType GetDevType(string Name)
        {
            foreach (var item in Enum.GetValues(typeof(DevType)))
            {
        ...
        public (DevParamsFromGet ParamsGet, object result) ParseFrame(RevData data)
        {
            ...
            switch ((DEV_GET_CMD_TYPE)data.cmd)
            {
                ...
                case DEV_GET_CMD_TYPE.M_ALL:
                    //todo
                    if (data.dataLen >= 10 * sizeof(float))
                    {
                        ...
                        pairs[DevParamsFromGet.BoardTemp] = BitConverter.ToSingle(data.data, 28);
                        pairs[DevParamsFromGet.AHT20Temp] = BitConverter.ToSingle(data.data, 32);
                        pairs[DevParamsFromGet.AHT20Humidity] = BitConverter.ToSingle(data.data, 36);
                        result = pairs;
                    }
                ...
                case DEV_GET_CMD_TYPE.BIT:
                    ...
                    break;
                case DEV_GET_CMD_TYPE.VERSION:
                    paramsGet = DevParamsFromGet.VERSION;
                    // result = BitConverter.ToString(data.data);
                    result = GetDevType(Encoding.UTF8.GetString(data.data));
                    break;
                case DEV_GET_CMD_TYPE.BUILDTIME:
                    paramsGet = DevParamsFromGet.BUILDTIME;
                    var timeString= BitConverter.ToUInt64(data.data).ToString();
                    result = DateTime.ParseExact(timeString, "yyyyMMddHHmm", null);
                    break;
