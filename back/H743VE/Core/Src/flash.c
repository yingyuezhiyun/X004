...
// 选定sector中的index 按照32Byte计算
uint16_t FlashSectorIndex = 0;
...
/*从指定地址开始写入数据*/
// FLASH_Addr:起始地址 32Byte 整数倍
// FLASH_Data:写入数据指针 （数据长度32Byte * N，不足补0）
// Size:写入数据长度  32Byte * N
int FLASH_WriteCfg(uint32_t Addr, uint8_t *Data, uint16_t Size)
{
    ...
    if (FlashSectorIndex > (sizeof(para_flash_area) / 32))
    {
        FLASH_Erase(CFG_ADDR);
        FlashSectorIndex = 0;
    }

    uint16_t flashdata_temp[16];
    // //首地址增加编程信息头
    Flash_info flash_info;
    flash_info.header = FLASH_PRAGMA_HEAD;
    flash_info.datasize = Size;
    memcpy((char *)flashdata_temp, &flash_info, sizeof(flash_info));
    // flashdata_temp[0]=FLASH_PRAGMA_HEAD;
    // flashdata_temp[1]=Size;
    memcpy(((char *)flashdata_temp) + sizeof(flash_info), Data, fmin(Size, sizeof(flashdata_temp) - sizeof(flash_info)));

    int data_pos = 32 - sizeof(flash_info);
    do
    {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, (CFG_ADDR + FlashSectorIndex * 32), /* (uint64_t) */ ((uint32_t)flashdata_temp)) != HAL_OK)
        {
            FlashSectorIndex++;
            reslt = -2;
            goto End;
...
int8_t read_data_withoutcheck(uint32_t ReadAddr, uint8_t *data, uint32_t Size)
{
...
int Get_Flash_Prag_Info(uint32_t addr)
{
    uint16_t Index = 0;
    while (1)
    {
        uint32_t temp = FLASH_ReadWord(addr + Index * 32);
        Flash_info *flash_info;
        flash_info = &temp;
        if (flash_info->header == FLASH_PRAGMA_HEAD) //
        {
            Index += ceil((sizeof(Flash_info) + flash_info->datasize) / 32.0);
        }
...
void Flash_Ini()
{
    FlashSectorIndex = Get_Flash_Prag_Info(CFG_ADDR);
}

void read_cfg(void)
{
    Flash_Ini();
    if (FlashSectorIndex == 0)
    {
        return;
    }
    read_data_withoutcheck(CFG_ADDR + (FlashSectorIndex - 1) * 32, &mem_cfg, sizeof(mem_cfg));
    // read_data(CFG_ADDR + (FlashSectorIndex - 1) * 32, &mem_cfg, sizeof(mem_cfg));
}
void write_cfg(void)
{

    FLASH_WriteCfg(CFG_ADDR, &mem_cfg, sizeof(mem_cfg));
}
