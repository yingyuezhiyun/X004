...
void mem_cfg_init(void)
{
    ...
    mem_cfg.app1_address = APP1_ADDR;
    mem_cfg.app2_address = APP2_ADDR;
    ...
    if (mem_cfg.app1_hdr.magic != FW_MAGIC && mem_cfg.app2_hdr.magic != FW_MAGIC)
    {
        mem_cfg.active_app = mem_cfg.app1_address;
        mem_cfg.app1_hdr.magic = FW_MAGIC;
        mem_cfg.app1_hdr.fw_size = 0;
        mem_cfg.app1_hdr.fw_crc = 0xFFFFFFFFU;
       // mem_cfg.app1_hdr.entry = 0;
        mem_cfg.app2_hdr.magic = FW_MAGIC;
        mem_cfg.app2_hdr.fw_size = 0;
        mem_cfg.app2_hdr.fw_crc = 0xFFFFFFFFU;
        //mem_cfg.app2_hdr.entry = 0;
...
/* If BOOT_DEBUG_SKIP_CFG_CHECK is defined, skip header+CRC check (for Keil debug). */
    ...
    if (app_addr == mem_cfg.app1_address)
        hdr = &mem_cfg.app1_hdr;
    else if (app_addr == mem_cfg.app2_address)
        hdr = &mem_cfg.app2_hdr;
    else
    {
        // unknown app region:
        return 0;
...
void Check_Jump_to_APP()
{
    ...
    if (mem_cfg.pending_state == PSTATE_APP_CHECK)
    {
        ...
        uint32_t other = 0;
        if (mem_cfg.active_app == mem_cfg.app1_address)
        {
           mem_cfg.app1_hdr.fw_size = 0; // invalidate
           other = mem_cfg.app2_address;
        }
        else
        {
            mem_cfg.app2_hdr.fw_size = 0; // invalidate
            other = mem_cfg.app1_address;
        }  
        if (check_app(other))
        {
            mem_cfg.active_app = other;
        }
        write_cfg();
    ...
    if (mem_cfg.pending_state == PSTATE_PENDING)
    {
        ...
        if (check_app(mem_cfg.active_app))
        {
            ...
            Jump_to_APP(mem_cfg.active_app);
            return;
        }
        else
        {
            info_printf("Pending active_app invalid. Marking failed.\r\n");
            mem_cfg.pending_state = PSTATE_FAILED; // failed
            if (mem_cfg.active_app == mem_cfg.app1_address)
            {
                mem_cfg.app1_hdr.fw_size = 0; // invalidate
    ...
    if (check_app(mem_cfg.app1_address))
    {
        info_printf("APP1 Check OK: 0x%08X\r\n", mem_cfg.app1_address);
        mem_cfg.active_app = mem_cfg.app1_address;
        write_cfg();
    ...
    {
        info_printf("APP1 Check Failed: 0x%08X\r\n", mem_cfg.app1_address);
    }

    if (check_app(mem_cfg.app2_address))
    {
        info_printf("APP2 Check OK: 0x%08X\r\n", mem_cfg.app2_address);
        mem_cfg.active_app = mem_cfg.app2_address;
        write_cfg();
    ...
    {
        info_printf("APP2 Check Failed: 0x%08X\r\n", mem_cfg.app2_address);
    }
...
void mem_cfg_show()
{
    ...
    info_printf("| pend :\t%-9s\t\t|\r\n", pstate_str);
    info_printf(" ---------------------------------------\r\n");
    info_printf("| app1 size:0x%08X crc:0x%08X\t|\r\n", mem_cfg.app1_hdr.fw_size, mem_cfg.app1_hdr.fw_crc);
    info_printf("| app2 size:0x%08X crc:0x%08X\t|\r\n", mem_cfg.app2_hdr.fw_size, mem_cfg.app2_hdr.fw_crc);
    info_printf(" ---------------------------------------\r\n");
    info_printf("\r\n");
}
