...
import argparse, struct, os, sys, re
...
def main():
    ...
    p.add_argument("--cfg-size", type=int, default=DEFAULT_CFG_SIZE, help="allocated cfg blob size in bytes to write at cfg-addr (used to pad/truncate the mem_cfg blob)")
    ...
    app1_hdr = (args.fw_magic, app1_size, app1_crc)
    app2_hdr = (0xFFFFFFFF, 0, 0xFFFFFFFF)  # default empty

    mem_cfg_blob = make_mem_cfg_bytes(args.bl_addr, args.app1_addr, app2_addr, args.cfg_addr, active_app, args.pending_state, app1_hdr, app2_hdr)
    # cfg-size 说明输出：按用户请求大小与实际mem_cfg大小取最大值，然后按32字节对齐，使用0x00填充不足部分
    min_needed = len(mem_cfg_blob)
    requested = args.cfg_size
    final_size = max(min_needed, requested)
    aligned_size = ((final_size + 31) // 32) * 32
    print(f"CFG: writing mem_cfg blob at 0x{args.cfg_addr:08X}, requested cfg-size={requested} bytes, mem_cfg={min_needed} bytes -> aligned size={aligned_size} bytes (pad with 0x00)")
    # pad 到 32-byte 对齐，使用 0x00
    if len(mem_cfg_blob) < aligned_size:
        mem_cfg_blob = mem_cfg_blob + bytes([0x00]) * (aligned_size - len(mem_cfg_blob))
    else:
        mem_cfg_blob = mem_cfg_blob[:aligned_size]

    regions.append((args.cfg_addr, mem_cfg_blob))
    ...
    if args.verify:
        ...
        try:
            ...
            sys.exit(1)
        parsed = parse_mem_cfg_from_map(memmap, args.cfg_addr)
        # print human-readable
        print("Parsed mem_cfg at 0x{:08X}:".format(args.cfg_addr))
        print("  bootloader_address : 0x{:08X}".format(parsed["boot"]))
        print("  app1_address       : 0x{:08X}".format(parsed["app1"]))
        print("  app2_address       : 0x{:08X}".format(parsed["app2"]))
        print("  cfg_address        : 0x{:08X}".format(parsed["cfg"]))
        print("  active_app         : 0x{:08X}".format(parsed["active"]))
        print("  pending_state      : 0x{:08X}".format(parsed["pending"]))
        print("  app1.magic         : 0x{:08X}".format(parsed["app1_magic"]))
        print("  app1.size          : 0x{:08X}".format(parsed["app1_size"]))
        print("  app1.crc           : 0x{:08X}".format(parsed["app1_crc"]))
        print("  app2.magic         : 0x{:08X}".format(parsed["app2_magic"]))
        print("  app2.size          : 0x{:08X}".format(parsed["app2_size"]))
        print("  app2.crc           : 0x{:08X}".format(parsed["app2_crc"]))

        # compare to generated expected values
        ok = True
        if parsed["boot"] != args.bl_addr:
            print("  MISMATCH bootloader_address (expected 0x{:08X})".format(args.bl_addr)); ok = False
        if parsed["app1"] != args.app1_addr:
            print("  MISMATCH app1_address (expected 0x{:08X})".format(args.app1_addr)); ok = False
        if parsed["cfg"] != args.cfg_addr:
            print("  MISMATCH cfg_address (expected 0x{:08X})".format(args.cfg_addr)); ok = False
        if parsed["app1_magic"] != args.fw_magic:
            print("  MISMATCH app1_magic (expected 0x{:08X})".format(args.fw_magic)); ok = False
        if parsed["app1_size"] != app1_size:
            print("  MISMATCH app1_size (expected 0x{:08X})".format(app1_size)); ok = False
        if parsed["app1_crc"] != app1_crc:
